  ...
  async function fetchWeatherData() {
    try {
      const weatherData = await fetch(
        `https://api.weatherapi.com/v1/forecast.json?days=3&key=72048723f49c411e8e795302240810&q=${city}`
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-gray-900 via-purple-900 to-blue-900 flex justify-center items-center">
      <div className="bg-gray-800 rounded-lg shadow-lg p-8 max-w-fit ">
        ...
        <form>
          <label htmlFor="city" className="text-white block text-start mb-2">
            Type City Name:
          </label>
          <input
            id="city"
          ...
          <div className="mt-6 grid grid-cols-2 gap-4 text-white">
            ...
            <div className="flex justify-center items-center p-3 border-[1px] rounded-lg border-gray-600">
              <p className="text-4xl font-bold">
                {weather.current.temp_c.toFixed()} °C
              </p>
            </div>
            <div className="flex justify-center items-center p-3 border-[1px] rounded-lg border-gray-600">
            ...
            <div className="flex justify-center items-center p-3 border-[1px] rounded-lg border-gray-600">
              <p className="text-md">
                Wind: {weather.current.wind_kph.toFixed()}kph
              </p>
            </div>
            <h2 className="col-span-2 font-semibold text-xl">5-day Forecast</h2>
            <div className="col-span-2 flex flex-col items-start p-3 border-[1px] min-w-fit rounded-lg gap-y-4 border-gray-600 text-white">
              {weather.forecast.forecastday.map((day: any) => (
                <Forecast
                  key={day.date}
                  date={weekdays(day.date)}
                  maxtemp={day.day.maxtemp_c.toFixed()}
                  mintemp={day.day.mintemp_c.toFixed()}
                  condition={conditionTrim(day.day.condition.text)}
